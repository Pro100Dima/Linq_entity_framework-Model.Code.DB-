    ...
    class Program
    {
        static void Main(string[] args)
        {
            using(mcsContext db = new mcsContext())
            {
                ...
                //TableStopReason sr = new TableStopReason(); // INSERT
                //sr.strReason = "";
                //
                //db.TablesStopReasons.Add(sr);
                //db.SaveChanges();

                //TableStopReason sr = db.TablesStopReasons.Find(1); // UPDATE
                //sr.strReason = "-*-";
                //db.SaveChanges();

                TableStopReason sr = db.TablesStopReasons.Find(1); // REMOVE
